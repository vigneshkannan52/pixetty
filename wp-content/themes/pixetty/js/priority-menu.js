(function () {
     ...
     */
    var navContainer = document.querySelector('.header-menus');
    var breaks = [];
     ...
     */
    if (!document.querySelector('#primary-menu') && !document.querySelector('#primary-menu-dropdown')) {
        return;
    }
    ...
    function updateNavigationMenu(container) {
         ...
         */
        if (!shouldUpdateMenu || !document.querySelector('#primary-menu')) {
            return;
        }
        // Adds the necessary UI to operate the menu.

        var visibleList = container.parentNode.querySelector('#primary-menu');
    ...
    function updateNavigationDropdownMenu(container) {

        /**
         * Let’s bail if our menu is empty.
         */
        if ( !shouldUpdateMenu || !document.querySelector('#primary-menu-dropdown')) {
            return;
        }
        // Adds the necessary UI to operate the menu.

        var visibleList = container.parentNode.querySelector('#primary-menu-dropdown');
        var menuHolder = container.querySelector('.primary-menu-wrapper');
        var hiddenList = visibleList.parentNode.nextElementSibling.querySelector('.hidden-links');
        var toggleButton = visibleList.parentNode.nextElementSibling.querySelector('.primary-menu-more-toggle');
    ...
    window.addEventListener('load', function () {
        updateSubMenuPosition(submenus);
        updateNavigationMenu(navContainer);
        updateNavigationDropdownMenu(navContainer);
    });
    ...
    var headerToggle = document.getElementById("header-toggle-button");
    if ( headerToggle ) {
        headerToggle.addEventListener("click", updateDropdownNavigation);
    }

    function updateDropdownNavigation() {
